...
/// Placeholder standing in for `oxibrowser_render::RenderDocument` when the
/// `native` feature is off (wasm build) — no Blitz pipeline is available, so
/// every `render_doc_cell` stays permanently `None` and these methods are
/// never actually invoked; they exist only so call sites written against the
/// native `RenderDocument` API still type-check under both configs.
#[cfg(not(feature = "native"))]
pub(crate) struct RenderDocument;

#[cfg(not(feature = "native"))]
impl RenderDocument {
    pub fn root_element_id(&self) -> usize {
        unreachable!("no RenderDocument exists in a wasm build")
    }
    pub fn query_selector(&self, _selector: &str) -> Option<usize> {
        None
    }
    pub fn query_selector_all(&self, _selector: &str) -> Vec<usize> {
        Vec::new()
    }
    pub fn tag_name(&self, _node: usize) -> Option<String> {
        None
    }
    pub fn node_attr(&self, _node: usize, _name: &str) -> Option<String> {
        None
    }
    pub fn node_attributes(&self, _node: usize) -> Vec<(String, String)> {
        Vec::new()
    }
    pub fn node_text(&self, _node: usize) -> String {
        String::new()
    }
    pub fn create_element(&mut self, _tag: &str) -> usize {
        unreachable!("no RenderDocument exists in a wasm build")
    }
    pub fn create_text_node(&mut self, _text: &str) -> usize {
        unreachable!("no RenderDocument exists in a wasm build")
    }
    pub fn append_child(&mut self, _parent: usize, _child: usize) {}
    pub fn set_attribute(&mut self, _node: usize, _name: &str, _value: &str) {}
    pub fn remove_attribute(&mut self, _node: usize, _name: &str) {}
    pub fn set_inline_style(&mut self, _node: usize, _property: &str, _value: &str) {}
    pub fn set_text(&mut self, _node: usize, _text: &str) {}
    pub fn remove_node(&mut self, _node: usize) {}
    pub fn node_layout_rect(&self, _node: usize) -> (f64, f64, f64, f64) {
        (0.0, 0.0, 0.0, 0.0)
    }
}
...
enum JsCommand {
    ...
    #[cfg(not(feature = "native"))]
    Capture { response_tx: Sender<JsResponse> },
...
pub struct JsRuntime {
    /// Command "channel". Native: a real `mpsc::Sender` to the background JS
    /// thread. wasm: `InlineCmdTx`, whose `send()` runs the command to
    /// completion on the spot against `engine` (see `InlineJsEngine`) —
    /// `wasm32-wasip1` has no working `thread::spawn` without a
    /// `wasi-threads`-capable host (confirmed empirically: panics with
    /// `Os { kind: Unsupported }` under wasmtime), so there is no background
    /// thread to send to. Every call site keeps calling
    /// `self.cmd_tx.send(JsCommand::X { .. })` unchanged in both configs.
    #[cfg(feature = "native")]
    ...
    /// wasm builds only: the JS engine lives directly on the caller's thread.
    /// `InlineCmdTx::send` calls straight into it via a `Weak`-free direct
    /// `Rc<RefCell<..>>` shared with `cmd_tx`.
    #[cfg(not(feature = "native"))]
    engine: Rc<RefCell<InlineJsEngine>>,
}

/// wasm-build stand-in for `mpsc::Sender<JsCommand>`: `send()` runs the
/// command to completion immediately against the shared `InlineJsEngine`
/// instead of handing it to a channel a background thread would drain.
/// Keeps every `self.cmd_tx.send(cmd)` call site identical to native.
#[cfg(not(feature = "native"))]
#[derive(Clone)]
struct InlineCmdTx {
    engine: Rc<RefCell<InlineJsEngine>>,
...
#[cfg(not(feature = "native"))]
impl InlineCmdTx {
    /// Runs `cmd` to completion inline. `Shutdown` is a legitimate no-op
    /// here (sent unconditionally by `JsRuntime::drop`) — there is no
    /// background thread loop for it to stop; dropping `Rc<RefCell<..>>`
    /// tears the engine down once every clone is gone.
    fn send(&self, cmd: JsCommand) -> std::result::Result<(), mpsc::SendError<JsCommand>> {
...
/// The `!Send` JS engine state, owned directly by `JsRuntime` under wasm
/// instead of living on a background thread. Mirrors the local variables
/// `js_thread_loop` builds once before its command loop.
#[cfg(not(feature = "native"))]
struct InlineJsEngine {
    ctx: Context,
...
impl InlineJsEngine {
    fn new(
        ...
        let fetch_tx_arc = Arc::new(RwLock::new(None));
        let local_storage_tx_arc = Arc::new(RwLock::new(None));
        let cookie_jar_arc = Arc::new(RwLock::new(None));
        let dom_snapshot = Arc::new(RwLock::new(None));
        let render_doc_cell = Rc::new(RefCell::new(None));
        let (ctx, job_queue) = create_context(
            console_output,
            &dom_snapshot,
            mutations,
            viewport,
...
impl JsRuntime {
    ...
    ///
    /// On native builds the JS engine (boa's `!Send` `Context`) runs on a
    /// dedicated background thread, matching a real browser's main-thread
    /// model; `cmd_tx` is the async bridge to it. `wasm32-wasip1` has no
    /// working thread spawn without a `wasi-threads`-capable host (confirmed
    /// empirically: `std::thread::spawn` panics with `Os { kind: Unsupported
    /// }` under wasmtime), so the wasm build instead runs the engine inline
    /// on the caller's thread — see `with_config` below and
    /// `Self::run_inline_until_shutdown_or_idle`.
    #[cfg(feature = "native")]
    ...
    /// wasm builds run the JS engine directly on the caller's thread — no
    /// background thread, no real channel. `cmd_tx` is an `InlineCmdTx`
    /// sharing `engine` by `Rc`, so every existing `self.cmd_tx.send(cmd)`
    /// call site runs the command to completion on the spot.
    #[cfg(not(feature = "native"))]
    pub fn with_config(config: JsRuntimeConfig) -> Self {
        let console_output = Arc::new(RwLock::new(Vec::<String>::new()));
        let mutations = Arc::new(RwLock::new(Vec::<DomMutation>::new()));
        let viewport = (config.viewport_width, config.viewport_height);
        let engine = Rc::new(RefCell::new(InlineJsEngine::new(
            &console_output,
            &mutations,
            viewport,
            &config.user_agent,
        )));
        let cmd_tx = InlineCmdTx {
            engine: engine.clone(),
    ...
    /// wasm builds have no Blitz render pipeline; capture is unsupported.
    #[cfg(not(feature = "native"))]
    pub async fn capture_png(&mut self, _opts: ()) -> Result<Vec<u8>> {
        Err(CoreError::ScreenshotError(
            "rendering unavailable in this build".into(),
        ))
    }
...
/// until a `Shutdown` is received. Native only — see `InlineJsEngine` for
/// the wasm equivalent that runs on the caller's thread.
#[cfg(feature = "native")]
fn js_thread_loop(
...
/// Run one `JsCommand` to completion, sending its response on the command's
/// own embedded `response_tx`. Returns `false` on `Shutdown` (caller should
/// stop looping), `true` otherwise. Shared by the native background-thread
/// loop (`js_thread_loop`) and the wasm inline dispatcher (`InlineCmdTx::send`).
///
/// `ctx`/`job_queue` are `&mut` (not owned) because a timed-out `Eval` can
/// rebuild both from scratch (`create_context`) via `*ctx = new_ctx` — the
/// engine survives a hung script by resetting, not by unwinding the caller.
#[allow(clippy::too_many_arguments)]
fn process_js_command(
    cmd: JsCommand,
    ctx: &mut Context,
    job_queue: &mut Rc<TokioJobQueue>,
    console_output: &Arc<RwLock<Vec<String>>>,
    mutations: &Arc<RwLock<Vec<DomMutation>>>,
    dom_snapshot: &Arc<RwLock<Option<DomSnapshot>>>,
    viewport: (u32, u32),
...
) -> bool {
    {
        match cmd {
            ...
            JsCommand::SetGlobal {
                ...
                let js_val = json_to_js_value(&value, ctx);
            ...
            JsCommand::SetDocument {
                ...
                // No Blitz render document in this build; still run the page's
            ...
            #[cfg(not(feature = "native"))]
            JsCommand::Capture { response_tx } => {
                let _ = response_tx.send(JsResponse::Error {
                    message: "rendering unavailable in this build".into(),
                });
            }
            JsCommand::Query {
                selector,
            ...
            #[cfg(not(feature = "native"))]
            JsCommand::GetDocumentSnapshot { response_tx, .. } => {
                let _ = response_tx.send(JsResponse::Error {
                    message: "rendering unavailable in this build".into(),
                });
            }
...
mod tests {
    ...
    async fn test_async_fetch_non_blocking() {
        // fetch() must NOT block the JS thread: a 5000 ms RTT must return to
        // the next statement in well under that. On `main` the recv() blocked
        // ≈ the full RTT. Threshold widened repeatedly (60ms/300ms RTT, then
        // 200ms/1000ms RTT) and both still flaked on GitHub Actions' shared
        // runners under contention even though the fetch was genuinely
        // non-blocking. Rather than keep chasing a tighter ratio, use a much
        // larger RTT (5000ms) with a fixed, generous 1000ms threshold: still
        // two orders of magnitude below the RTT (a real block would fail by
        // ~5000ms, not by a few hundred), but the absolute margin now easily
        // absorbs realistic scheduler jitter.
        let mut rt = JsRuntime::new();
        ...
        let _h = spawn_test_fetch_handler(req_rx, resp_tx, 5000, "{\"value\":42}".to_string());

        let r = rt
            .evaluate(
                "window.__t0 = performance.now(); fetch('http://x/'); window.__t1 = performance.now(); (window.__t1 - window.__t0) < 1000",
            )
            .await
            .unwrap();
