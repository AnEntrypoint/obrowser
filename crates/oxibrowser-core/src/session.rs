...
pub struct Session {
    ...
    /// WebSocket bridge task handle (for cleanup). Only spawned under `native`.
    #[allow(dead_code)]
    #[cfg(feature = "native")]
...
impl Session {
    ...
    pub async fn new(
        ...
        let fetch_task: Option<std::thread::JoinHandle<()>> = {
            drop((fetch_rx, fetch_resp_tx));
            None
        };

        // Spawn WebSocket bridge handler thread (Phase 4) — native only.
        #[cfg(feature = "native")]
        let ws_task = Some(std::thread::spawn(move || {
            handle_ws_requests(ws_req_rx, ws_event_tx);
        }));
        #[cfg(not(feature = "native"))]
        drop((ws_req_rx, ws_event_tx));

        // Spawn localStorage sync handler thread (native only; wasip1 has no
        // real OS thread spawn).
        let local_storage_arc =
            Arc::new(parking_lot::RwLock::new(std::collections::HashMap::new()));
        #[cfg(feature = "native")]
        let local_storage_task = {
        ...
        #[cfg(not(feature = "native"))]
        let local_storage_task: Option<std::thread::JoinHandle<()>> = {
            drop(ls_rx);
            None
        };
    ...
    /// Navigate to a URL. Real HTTP fetches go through `HttpClient`, which
    /// under wasm proxies to the JS host via `host_fetch` (see
    /// `crate::network::host_abi`) instead of a native TLS/socket stack.
    #[cfg(not(feature = "native"))]
    pub async fn navigate(&mut self, url: &str) -> Result<()> {
        if self.closed.load(Ordering::SeqCst) {
            return Err(CoreError::SessionClosed);
        ...
        let ct_header = crate::network::client::header_lookup(&response.headers, "content-type")
            .unwrap_or("text/html")
            .to_string();
        let max = self.config.max_response_body_bytes;
        let (bytes, truncated) = HttpClient::read_body_limited(response, max).await?;
    ...
    pub async fn go_back(&mut self) -> Result<()> {
        ...
        if self.history_index == 0 {
            return Err(CoreError::NavigationFailed("no previous page".into()));
        }
        self.history_index -= 1;
        let url = self.history[self.history_index].clone();
        self.refetch_history_entry(url).await
    }
    ...
    pub async fn go_forward(&mut self) -> Result<()> {
        ...
        if self.history_index >= self.history.len().saturating_sub(1) {
            return Err(CoreError::NavigationFailed("no next page".into()));
        }
        self.history_index += 1;
        let url = self.history[self.history_index].clone();
        self.refetch_history_entry(url).await
    ...
    pub async fn reload(&mut self) -> Result<()> {
        ...
        let Some(url) = self.current_url().cloned() else {
            return Err(CoreError::NavigationFailed("no current page".into()));
        };
        self.refetch_history_entry(url).await
    }

    /// Shared re-fetch body for `go_back`/`go_forward`/`reload` (wasm build):
    /// fetch `url` via the host-proxied `HttpClient`, build a fresh `Page`,
    /// and re-inject the DOM snapshot — without touching navigation history.
    #[cfg(not(feature = "native"))]
    async fn refetch_history_entry(&mut self, url: Url) -> Result<()> {
        let _in_flight = InFlightGuard::new(self.in_flight.clone());
        let response = self.http_client.fetch(&url).await?;
        let ct_header = crate::network::client::header_lookup(&response.headers, "content-type")
            .unwrap_or("text/html")
    ...
    pub async fn post(&mut self, url: &str, body: &str, content_type: &str) -> Result<()> {
        ...
        let status = response.status;
        let final_url = Url::parse(&response.url).unwrap_or_else(|_| parsed.clone());
        let ct = crate::network::client::header_lookup(&response.headers, "content-type")
            .unwrap_or("text/html")
            .to_string();
    ...
    pub async fn capture_screenshot_png(&mut self, _viewport_width: u32) -> Result<Vec<u8>> {
        Err(CoreError::ScreenshotError(
            "rendering unavailable in wasm build".into(),
        ))
    }
