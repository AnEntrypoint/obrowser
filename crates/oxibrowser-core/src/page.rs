...
impl Page {
    ...
    /// wasm builds have no Blitz render pipeline; screenshots are unsupported.
    #[cfg(not(feature = "native"))]
    pub fn to_screenshot_png(&self, _viewport_width: u32) -> Result<Vec<u8>> {
        Err(CoreError::ScreenshotError(
            "screenshot unsupported without native feature".into(),
        ))
    }
    ...
    /// wasm builds have no Blitz render pipeline; screenshots are unsupported.
    #[cfg(not(feature = "native"))]
    pub fn to_screenshot_png_live(&self, _viewport_width: u32) -> Result<Vec<u8>> {
        Err(CoreError::ScreenshotError(
            "screenshot unsupported without native feature".into(),
        ))
    }
