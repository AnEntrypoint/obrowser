...
/// Stub HTTP response used when the `native` feature is disabled (wasm build).
/// Network calls are unavailable in-guest; every `HttpClient` method returns
/// `Err` before a value of this type is ever constructed. Callers that pattern
/// -match on a real response (status/headers/body) are gated `native`-only at
/// the call site instead of reimplementing the wreq::Response surface here.
...
#[cfg(not(feature = "native"))]
impl Response {
    pub fn status(&self) -> StatusStub {
        StatusStub(self.status)
    }
    ...
    pub async fn bytes(self) -> std::result::Result<Vec<u8>, CoreError> {
        Ok(self.body)
    }
}

/// Minimal `StatusCode`-alike so wasm call sites written against
/// `wreq::StatusCode`'s `.as_u16()` still work unchanged.
#[cfg(not(feature = "native"))]
#[derive(Debug, Clone, Copy)]
pub struct StatusStub(u16);
...
/// Stub HTTP client type when the `native` feature is disabled (wasm build).
#[cfg(not(feature = "native"))]
...
/// wasm build: network I/O has no native TLS/socket stack in-guest, so every
/// request is proxied to the JS host via the `host_fetch` WASI import (see
/// `crate::network::host_abi`) — the host performs the real HTTP round-trip
/// and hands back a decoded response.
#[cfg(not(feature = "native"))]
impl HttpClient {
    pub fn new(config: &BrowserConfig, cookie_jar: Arc<RwLock<CookieJar>>) -> Result<Self> {
        let ip_filter = if config.enable_ssrf_filter {
    ...
    pub(crate) async fn read_body_limited(
        ...
        if response.body.len() > max_bytes {
            Ok((response.body[..max_bytes].to_vec(), true))
        } else {
            Ok((response.body, false))
        }
    }

    pub async fn fetch(&self, url: &Url) -> Result<Response> {
        self.host_request(url, "GET", None).await
    }
    ...
    pub async fn fetch_text(&self, url: &Url) -> Result<String> {
        ...
        let content_type = response
            ...
            .map(|(_, v)| v.as_str());
        Ok(crate::encoding::decode_html(&response.body, content_type))
    }

    pub async fn post(&self, url: &Url, body: Vec<u8>) -> Result<Response> {
        self.host_request(url, "POST", Some(body)).await
    }

    pub async fn post_json(&self, url: &Url, json: &serde_json::Value) -> Result<Response> {
        self.host_request(url, "POST", Some(json.to_string().into_bytes()))
            .await
    }
    ...
    pub async fn post_form(&self, url: &Url, form: &[(&str, &str)]) -> Result<Response> {
        let body = form
            ...
            .collect::<Vec<_>>()
            .join("&");
        self.host_request(url, "POST", Some(body.into_bytes()))
            .await
    }
