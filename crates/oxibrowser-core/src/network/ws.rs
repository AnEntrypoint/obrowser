...
/// Stub for wasm builds — WebSocket I/O is unavailable until proxied to a
/// host `host_ws_connect` import; immediately reports connect failure.
#[cfg(not(feature = "native"))]
pub async fn run_ws_connection(
    id: u64,
    _url: String,
    _protocols: Vec<String>,
    _cmd_rx: tokio::sync::mpsc::Receiver<WsCmd>,
    event_tx: std::sync::mpsc::Sender<WsEvent>,
) {
    let _ = event_tx.send(WsEvent::Error {
        id,
        message: "websocket not supported in this build".into(),
    });
    let _ = event_tx.send(WsEvent::Close {
        id,
        code: 1006,
